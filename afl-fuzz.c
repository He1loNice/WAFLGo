...
EXP_ST u8* trace_bits_c;                /* SHM with instrumentation bitmap  */
...
EXP_ST u8  virgin_bits[MAP_SIZE],     /* Regions yet untouched by fuzzing */
           target_intersectionBits[MAP_SIZE>>3],
...
static s32 shm_id_c;                    /* ID of the SHM region             */
...
EXP_ST u32 queued_paths,              /* Total number of queued testcases */
           ...
           temp_current_entry=0,
           havoc_div = 1;             /* Cycle count divisor for havoc    */

EXP_ST u32 queued_paths2,              /* Total number of queued testcases */
           pending_not_fuzzed2;        /* Queued but not done yet          */
...
static u64 total_bitmap_size,         /* Total bit count for all bitmaps  */
           total_bitmap_size_suf,      /* Number of bitmaps counted        */
           total_bitmap_size_suf_target,      /* Number of bitmaps counted        */
           total_bitmap_entries,      /* Number of bitmaps counted        */
           total_bitmap_entries_suf,      /* Number of bitmaps counted        */
           total_bitmap_entries_suf_target;      /* Number of bitmaps counted        */
...
struct queue_entry {
  ...
  u32 number;                          /* File name for the test case      */
  ...
  u8 *target_branch_trace;
  u8 *target_branch_trace_target[TARGETS_NUM];
  u8 *target_branch_trace_target_data[TARGETS_NUM];
  u32 tc_ref;                         /* Trace bytes ref count            */

  double distance;                    /* Distance to targets              */
  double distance_target[TARGETS_NUM];                    /* Distance to targets              */
  double suffix_cnt;                                   /* Distance to targets              */

  u8 is_suf_target;                   
  u8 is_target;
  u8 is_rare_target;                  
  u8 is_rare_target_suffix;           
  u32 rarest_num;                      
  u32 rarest_id;                     
  u8 *trace_bb;
...
struct Q2 {
  struct queue_entry *queue_this;

  struct Q2 *queue_next;
};

static struct Q2  *queue2=NULL,
                  *queue2_top=NULL,
                  *queue2_cur=NULL;

static struct queue_entry *queue_temp = NULL;
...
EXP_ST u64 Exit_seed = 0;


static double cur_distance = -1.0;     /* Distance of executed input       */
static double cur_distance_target[TARGETS_NUM] = {-1.0};
static double max_distance = -1.0;     /* Maximal distance for any input   */
static double max_distance_target[TARGETS_NUM] = {-1.0}; 
static double min_distance = -1.0;     /* Minimal distance for any input   */
static double min_distance_target[TARGETS_NUM] = {-1.0}; 
static double cur_suffix = -1.0;     /* Distance of executed input       */
static double max_suffix = -1.0;     /* Maximal distance for any input   */
static double min_suffix = -1.0;     /* Minimal distance for any input   */

static u32 t_x = 10;                  /* Time to exploitation (Default: 10 min) */

static u8* (*post_handler)(u8* buf, u32* len);

static u32 HITCOUNT_THRESHOLD = 10000;
static u32 CO_HITCOUNT_THRESHOLD = 2000;
static u32 MIN_NUMHIT = 1;
/* @RB@ Things about branches */

static u32 vanilla_afl = 1000;      /* @RB@ How many executions to conduct 
                                         in vanilla AFL mode               */
static u32 Inter_TargetSeedNum = 256;
...
static u8 run_in_Q2 = 0;

static u32 target_exec = 0;

static u32 seed_num = 0;
...
static u32 targets_num_orig = 0;
static u32 targets_branch_num = 0;
static u32 targets_branch_num_target[TARGETS_NUM] = {0};
static u32 targets_branch_num_target_data[TARGETS_NUM] = {0};
u32 order_edge[MAX_RARE_BRANCHES] = {0};
u32 order_edge_target[TARGETS_NUM][MAX_RARE_BRANCHES] = {0};
u32 order_edge_target_data[TARGETS_NUM][MAX_DATA_SUFFIX] = {0};
int target_ids[TARGETS_NUM] = {1};

u32 edge_curloc[MAP_SIZE] = {0};
u32 edge_distance[MAP_SIZE] = {(u32)(-1)};

struct CE{
  u32 edgeidx;
  struct CE *next;
};
struct CE *curloc_edge[MAP_SIZE] = {NULL};

static u8* targets_bits;

static u32 *target_suffix;  /*now:map<target_id,suffix_num>*/ /*todo::map<target_id,set<suffix_id>>*/
u32 *suffix_bb_execs; /*map<suf_id,execs>*/
u32 *rare_suffix_bb;  /*set<suf_id>*/
u32 total_suffix_num;
u32 rare_suffix_num;  

u32 distance_val[MAP_SIZE] = {(u32)(-1)};
u32 branch_distance[MAP_SIZE] = {(u32)(-1)};
u32 branch_flag[MAP_SIZE] = {0};
...
void fileonly (char const *fmt, ...) { 
    ...
    char *debugEnv = getenv("DEBUG1");
    if (debugEnv != NULL && f == NULL) {
...
void keep(struct queue_entry* q , u8 *trace_bits_suf_bb){
  for (int i = 0; i < total_suffix_num;i++){
    if(trace_bits_suf_bb[i]>0){
      q->trace_bb[i] = 1;
    }
  }
}
...
static u32 *is_target_branch_ids(u8 *trace_bits_mini,int target_index);
...
static u32 get_random_modifiable_posn(u32 num_to_modify, u8 mod_type, u32 map_len, u8* branch_mask, u32 * position_map){
  ...
  for (int i = 0; i < map_len; i ++){
    if ((branch_mask[i] & mod_type) || (UR(100)==0)){
...
static void add_to_queue_2(struct queue_entry* q) {
  struct Q2* q2 = ck_alloc(sizeof(struct Q2));
  q2->queue_this = q;
  q2->queue_next = NULL;

  if (queue2 && queue2_top) {

    queue2_top->queue_next = q2;
    queue2_top = q2;

  } else {    

    queue2 = queue2_top = q2;

  }

}

static void set_target_branch_flag(u32 * target_branch_hit){
  int ii;
  for (ii = 0; (ii<MAX_RARE_BRANCHES) && (target_branch_hit[ii] != 0); ii++)
  {
    branch_flag[target_branch_hit[ii] -1 ] = 1;
    // DEBUG1("set branch_flag[%d] =1 \n", target_branch_hit[ii] - 1);
  }
}


static void unset_target_branch_flag(u32 * target_branch_hit){
  if(target_branch_hit ==NULL){
    return;
  }
  int ii;
  for (ii = 0; (ii<MAX_RARE_BRANCHES) && (target_branch_hit[ii]) != 0; ii++)
  {
    // DEBUG1("unset branch_flag[%d] =0\n", target_branch_hit[ii] - 1);
    branch_flag[target_branch_hit[ii] -1 ] = 0;
  }
}

int get_rarest_target(){
  u32 min = target_count[0];
  int index = 0;
  for (int i = 0; i < targets_num;i++){
    if(target_count[i]<min){
      min = target_count[i];
      index = i;
    }
...
static void add_to_queue(u8* fname, u32 len, u8 passed_det,u32 number) {
  ...
  // DEBUG1("q->trace_mini == NULL : fname: %s \n", fname);
  }
  q->fuzzed_branches = ck_alloc(MAP_SIZE >> 3);

  for (int i = 0; i < TARGETS_NUM;i++){
    q->target_branch_trace_target[i] = ck_alloc(MAX_RARE_BRANCHES);
    u8 *temp_target = (u8 *)q->target_branch_trace_target[i];
    for (int j = 0; j < targets_branch_num_target[i];j++){
      u8 flag_target = *(trace_bits + MAP_SIZE + 16 + 16 + 16 + 16 + TARGETS_NUM * 16 + targets_num + MAX_RARE_BRANCHES * (i+1) + j);
  ...
  for (int i = 0; i < TARGETS_NUM;i++){
    q->target_branch_trace_target_data[i]=ck_alloc(MAX_DATA_SUFFIX);
    u8 *temp_target = (u8 *)q->target_branch_trace_target_data[i];
    for (int j = 0; j < targets_branch_num_target_data[i];j++){
      u8 flag_target = *(trace_bits + MAP_SIZE + 16 + 16 + 16 + 16 + TARGETS_NUM * 16 + targets_num + MAX_RARE_BRANCHES * (targets_num + 1) + MAX_DATA_SUFFIX * (i + 1) + j);
      if(flag_target > 0){
        temp_target[j] != 1;
      }
    }
    ...
    q->target_branch_trace = ck_alloc(MAX_RARE_BRANCHES);
  u8 *temp = (u8 *)q->target_branch_trace;
  for(int i=0;i<targets_branch_num;i++){
    u8 flag = *(trace_bits + MAP_SIZE + 16 + 16 + 16 + 16 + TARGETS_NUM * 16 + targets_num + i);
    if(flag > 0){
      // DEBUG1("target_branch:%u been hit\n", i);
      temp[i] |= 1 ;
    }
  }
  // @End

  int rearest_target=  get_rarest_target();

  q->queue_target_branch_hit = is_target_branch_ids(q->target_branch_trace_target[rearest_target],rearest_target);
  if(q->queue_target_branch_hit!=NULL){
  DEBUG1("add to queue 2, q->fname:%s\n",fname);
    add_to_queue_2(q);
    set_target_branch_flag(q->queue_target_branch_hit);
  }


    u32 i;
    for (i=0; i<targets_num; i++) {
      u8 flag = *(trace_bits + MAP_SIZE + 16 + 16 + 16 + 16 + TARGETS_NUM * 16 + i);
      if (flag) {
  ...
  if(is_rare_target){
    DEBUG1("rare add to queue 2, q->fname:%s\n",fname);
    add_to_queue_2(q);
  }
  ...
  q->trace_bb = ck_alloc(total_suffix_num);
  keep(q, trace_bits_suf_bb);

  q->distance = cur_distance;  
  for(int i = 0; i < targets_num;i++){
    q->distance_target[i] = cur_distance_target[i];
  }
  if (cur_distance > 0) {
  ...
  for(int i = 0; i < targets_num;i++){
    if (cur_distance_target[i] > 0) {
      if (max_distance_target[i] <= 0) {
        max_distance_target[i] = cur_distance_target[i];
        min_distance_target[i] = cur_distance_target[i];
      }
      if (cur_distance_target[i] > max_distance_target[i]) max_distance_target[i] = cur_distance_target[i];
      if (cur_distance_target[i] < min_distance_target[i]) min_distance_target[i] = cur_distance_target[i];
    }
  }


  q->suffix_cnt = cur_suffix;
  if (cur_suffix > 0) {

    if (max_suffix <= 0) {
      max_suffix = cur_suffix;
      min_suffix = cur_suffix;
    }
    if (cur_suffix > max_suffix) max_suffix = cur_suffix;
    if (cur_suffix < min_suffix) min_suffix = cur_suffix;
...
static inline u8 has_new_bits(u8* virgin_map) {
  ...
  u64 *total_distance_target[TARGETS_NUM];
  u64 *total_count_target[TARGETS_NUM];
  for (int i = 0; i < targets_num;i++){

    total_distance_target[i] = (u64 *)(trace_bits + MAP_SIZE + 64 + 16 * i);
    total_count_target[i] = (u64 *)(trace_bits + MAP_SIZE + 8 + 64 + 16 * i);
  }
  ...
  for(int i = 0; i < targets_num;i++){
    if (*total_count_target[i] > 0 && *total_distance_target[i] >0){
      cur_distance_target[i] = (double) (*total_distance_target[i] ) / (double) (*total_count_target[i]);
    }else{
      cur_distance_target[i] = -1.0;
    }
  }

  /* Calculate suffix of current input to targets */
  u64* total_suffix = (u64*) (trace_bits + MAP_SIZE + 32);
  u64* Runtarget = (u64 *)(trace_bits + MAP_SIZE + 24);

  if (*total_suffix > 0 && *Runtarget)
    cur_suffix = (double)(*total_suffix);
  else
    cur_suffix = -1.0;
  ...
  /* Calculate distance of current input to targets */
  u32* total_suffix = (u32*) (trace_bits + MAP_SIZE + 16);
  u32* Runtarget = (u32 *)(trace_bits + MAP_SIZE + 12);
  if (*total_suffix > 0 && *Runtarget)
    cur_suffix = (double)(*total_suffix);
  else
    cur_suffix = -1.0;
    ...
    if (trace_bits[i]) {
       ...
       if (top_rated[i]) {
        ...
        if(is_suf){
          ...
          if(top_rated[i]->is_suf_target){
            continue;
          }else if(fav_factor > top_rated[i]->exec_us * top_rated[i]->len){
            continue;
          }
        }
...
EXP_ST void setup_shm(void) {
  ...
  if (!in_bitmap) {
    ...
    memset(target_intersectionBits, 255, MAP_SIZE>>3);
    memset(virgin_bits_suf, 255, MAP_SIZE);
    memset(virgin_bits_suf_target, 255, MAP_SIZE);
    memset(virgin_bits_suf_bb, 255, MAP_SIZE);
  }
  ...
  if(target_count == NULL){
    FATAL("target_count is null\n");
  }

  target_count_orig = ck_alloc(targets_num_orig * sizeof(u32));
  if(target_count_orig == NULL){
    FATAL("target_count_orig is null\n");
  }

  suffix_bb_execs = ck_alloc(total_suffix_num * sizeof(u32));
  rare_suffix_bb = ck_alloc(total_suffix_num  * sizeof(u32));

  /* Allocate 24 byte more for distance info */
  shm_id = shmget(IPC_PRIVATE, MAP_SIZE + 16 + 16 + 16 + 16 + TARGETS_NUM * 16 + targets_num + MAX_RARE_BRANCHES * (targets_num+1) + MAX_DATA_SUFFIX * (targets_num+1), IPC_CREAT | IPC_EXCL | 0600);
  shm_id_c = shmget(IPC_PRIVATE, MAP_SIZE + 16 + 16 + 16 + 16 , IPC_CREAT | IPC_EXCL | 0600);
  shm_id_suf = shmget(IPC_PRIVATE, MAP_SIZE + 16 + 16 + 16 + 16 , IPC_CREAT | IPC_EXCL | 0600);
  shm_id_suf_bb = shmget(IPC_PRIVATE, MAP_SIZE + 16 + 16 + 16 + 16 , IPC_CREAT | IPC_EXCL | 0600);

  if (shm_id < 0) PFATAL("shmget() failed");
  if (shm_id_c < 0) PFATAL("shmget() failed");
  ...
  shm_str_c = alloc_printf("%d", shm_id_c);
  ...
  if (!dumb_mode) setenv(SHM_ENV_VAR_C, shm_str_c, 1);
...
static void read_testcases(void) {
  ...
  for (i = 0; i < nl_cnt; i++) {
    ...
    add_to_queue(fn, st.st_size, passed_det,i);
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  memset(trace_bits, 0, MAP_SIZE + 16 + 16 + 16 + 16 + TARGETS_NUM * 16 + targets_num + MAX_RARE_BRANCHES * (targets_num+1) + MAX_DATA_SUFFIX * (targets_num+1));
  memset(trace_bits_suf, 0, MAP_SIZE + 16 + 16 + 16 + 16);
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    // DEBUG1("Running target binary;%s\n",target_path);
...
static void find_rare_suffix_bb(){
  u32 *temp;
  temp = ck_alloc(total_suffix_num * sizeof(u32));
  memcpy(temp, suffix_bb_execs, sizeof(u32) * total_suffix_num);
  memset(rare_suffix_bb, total_suffix_num + 1, sizeof(u32) * total_suffix_num );

  int num = 0;
  int i = 0;
  int zero_num = 0;

  qsort(temp, total_suffix_num, sizeof(u32), comp);
  for ( i = 0; ((num < (total_suffix_num / 100)) && (i < total_suffix_num));)
  {
    for (int j = 0; j < total_suffix_num;j++){
      if(temp[i]==0){
        i++;
        zero_num++;
        continue;
      }
      if(suffix_bb_execs[j]==temp[i]){
        rare_suffix_bb[num] = j;
        num++;
        i++;
      }
    }
  ...
  if(i<total_suffix_num ){
    fprintf(mutation_log, "\nzero_num: %d, i = : %d, rarest_execs: %d, second rarest_execs: %d->%d\n",zero_num,i, temp[i-num], temp[i-num+1], temp[i-1]);
  }
  fprintf(mutation_log, "\nfind_rare_suffix_bb: %d; rarest_execs: %d\n",rare_suffix_num, temp[i- 1]);
  // for(int i=0;i<rare_suffix_num;i++){
...
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    for (i=0; i<targets_num_orig; i++) {
      u64 * flag = (u64 *)(trace_bits + MAP_SIZE + 24 );
      uint64_t value = (u64 ) (*flag);
      uint64_t bit = (value >> i) & 1;
      if (bit) {
        if (target_count_orig[i] < BIG_NUMBER)
          target_count_orig[i]++;
        if (target_count_orig[i] < TARGET_LIMIT)          
          is_rare_target = 1;
      }
    }
  ...
  u64* aroundTarget = (u64*) (trace_bits + MAP_SIZE + 16);
  u64 *not_preSeed = (u64 *)(trace_bits + MAP_SIZE + 48);
  q->ttarget_excutes = (u64 ) (*targetSeed);
  q->crash_excutes = (u64 ) (*crashSeed);
  q->around_target = (u64)(*aroundTarget);
  q->not_pre = (u64)(*not_preSeed);
#else
  u32* targetSeed = (u32 *)(trace_bits + MAP_SIZE + 12);
  u32* aroundTarget = (u64*) (trace_bits + MAP_SIZE + 8);
  u32 *not_preSeed = (u64 *)(trace_bits + MAP_SIZE + 24);

  q->ttarget_excutes = (u32) (*targetSeed);
  q->around_target = (u32)(*aroundTarget);
  q->not_pre = (u32)(*not_preSeed);
  ...
  total_bitmap_size_suf += q->bitmap_size_suffix;
  total_bitmap_size_suf_target += q->bitmap_size_suffix_target;

  total_bitmap_entries++;
  total_bitmap_entries_suf++;
  total_bitmap_entries_suf_target++;
...
static void perform_dry_run(char** argv) {
  ...
  while (q) {
    ...
    for (int i = 0; i < TARGETS_NUM;i++){
      ck_free(q->target_branch_trace_target[i]);
      q->target_branch_trace_target[i] = ck_alloc(MAX_RARE_BRANCHES);
      u8 *temp_target = (u8 *)q->target_branch_trace_target[i];
      for (int j = 0; j < targets_branch_num_target[i];j++){
        u8 flag_target = *(trace_bits + MAP_SIZE + 16 + 16 + 16 + 16 + TARGETS_NUM * 16 + targets_num + MAX_RARE_BRANCHES * (i+1) + j);
    ...
    for (int i = 0; i < TARGETS_NUM;i++){
      ck_free(q->target_branch_trace_target_data[i]);
      q->target_branch_trace_target_data[i]=ck_alloc(MAX_DATA_SUFFIX);
      u8 *temp_target = (u8 *)q->target_branch_trace_target_data[i];
      for (int j = 0; j < targets_branch_num_target_data[i];j++){
        u8 flag_target = *(trace_bits + MAP_SIZE + 16 + 16 + 16 + 16 + TARGETS_NUM * 16 + targets_num + MAX_RARE_BRANCHES * (targets_num + 1) + MAX_DATA_SUFFIX * (i + 1) + j);
    ...
    if(q->is_rare_target>0){
      ...
      DEBUG1("dry_run add to queue 2, q->fname:%s\n",q->fname);
      add_to_queue_2(q);
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    s32 suf_target_seeds = 0;
    bool is_suf_target_seeds = false;
    if(tt>0){
      hnb_suffix_target = presuf_has_new_bits(virgin_bits_suf_target, trace_bits_suf);
      presuf_has_new_bits(virgin_bits_suf_bb, trace_bits_suf_bb);
    }
    presuf_has_new_bits(virgin_bits_suf, trace_bits_suf);
    ...
    if (!(hnb = has_new_bits(virgin_bits)) ) {
      if(!hnb_suffix_target){
        if (crash_mode) total_crashes++;
        return 0;
      }else{
        suf_target_seeds++;
        is_suf_target_seeds = true;
      }
    }    
    ...
    if(! is_suf_target_seeds){
      ...
      fn = alloc_printf("%s/queue/id:%06u,%llu,%s,is_suf_target", out_dir, queued_paths,
                        get_cur_time() - start_time,
                        describe_op(hnb));
      fn_target = alloc_printf("%s/target/id:%06u,%llu,%s,is_suf_target", out_dir, queued_paths,
                               get_cur_time() - start_time,
                               describe_op(hnb));
    }
    ...
    fn_target = alloc_printf("%s/queue/id_%06u", out_dir, queued_paths);

#endif /* ^!SIMPLE_FILES */

    add_to_queue(fn, len, 0,queued_paths);
...
#else
      if(tt){
        fn = alloc_printf("%s/crashes/target_id_%06llu_%02u", out_dir, unique_crashes,
                          kill_signal);
      }else{
...
static void write_stats_file(double bitmap_cvg,double sufbit_cvg,double tarsufbit_cvg,double tarsufbitbb_cvg, double stability, double eps) {
  ...
  fprintf(f, "start_time        : %llu\n"
             ...
             queue_cycle ? (queue_cycle - 1) : 0, total_execs,Exit_seed, eps,
...
#define SP20    SP10 SP10
  ...
  SAYF(bV bSTOP "  exit execs : " cRST "%-21s " bSTG bV bSTOP 
       " favored paths : " cRST "%-22s " bSTG bV "\n", DI(Exit_seed), tmp);
...
static u32 calculate_score(struct queue_entry* q, int test_targetid) {

  u32 avg_exec_us = total_cal_us / total_cal_cycles;
  u32 avg_bitmap_size = total_bitmap_size / total_bitmap_entries;
  u32 avg_bitmap_size_suf = total_bitmap_size_suf / total_bitmap_entries_suf;
  u32 avg_bitmap_size_suf_target = total_bitmap_size_suf_target / total_bitmap_entries_suf_target;
  u32 perf_score = 100;
  ...
  fprintf(distance_log, " perf_score = %4d\n", perf_score);

  if(q->not_useful_seed && !q->has_new_cov){
    perf_score *= 0.01;
    fprintf(distance_log, "seed-not-useful perf_score = %4d\n", perf_score);
  }
  ...
  double power_factor = 1.0;
    ...
    if ((q->distance_target[test_targetid] > 0) && (target_count[test_targetid] == 0)) {
    ...
    if (max_distance_target[test_targetid] != min_distance_target[test_targetid])
      normalized_d = (q->distance_target[test_targetid] - min_distance_target[test_targetid]) / (max_distance_target[test_targetid] - min_distance_target[test_targetid]);
  ...
  // fprintf(distance_log, "[Time %llu] test_targetid: %d,q->distance: %4lf, max_distance: %4lf min_distance: %4lf, T: %4.3lf, power_factor: %4.3lf, adjusted perf_score: %4d\n", t,test_targetid, q->distance_target[test_targetid], max_distance_target[test_targetid], min_distance_target[test_targetid], T, power_factor, perf_score);


  if (q->bitmap_size_suffix_target * 0.3 > avg_bitmap_size_suf_target) perf_score *= 3;
  else if (q->bitmap_size_suffix_target * 0.5 > avg_bitmap_size_suf_target) perf_score *= 2;
  else if (q->bitmap_size_suffix_target * 0.75 > avg_bitmap_size_suf_target) perf_score *= 1.5;
  else if (q->bitmap_size_suffix_target * 3 < avg_bitmap_size_suf_target) perf_score *= 0.25;
  else if (q->bitmap_size_suffix_target * 2 < avg_bitmap_size_suf_target) perf_score *= 0.5;
  else if (q->bitmap_size_suffix_target * 1.5 < avg_bitmap_size_suf_target) perf_score *= 0.75;
  // fprintf(distance_log, "   after suffix q->suf:%d, avg:%d, perf_score = %4d\n",q->bitmap_size_suffix_target,avg_bitmap_size_suf_target, perf_score);
  ...
  // fprintf(distance_log, "after all ,q->name:%s power_factor = %4f, perf_score = %4d\n\n", q->fname,power_factor, perf_score);
...
static u32 * is_target_branch_ids_data(u8* branch_trace, int target_index){
  if(branch_trace==NULL){
    return NULL;
  }
  u32 * branch_ids = ck_alloc(sizeof(u32) * MAX_DATA_SUFFIX);
  u32 * branch_ids_hits = ck_alloc(sizeof(u32) * MAX_DATA_SUFFIX);
  int min_hit_index = 0;

  for (int i = 0; i < targets_branch_num_target_data[target_index];i++){
    if((branch_trace[i] ) == 1){
      u32 edge_index = order_edge_target_data[target_index][i];
      DEBUG1("edge_index: %d\n", edge_index);
      if (contains_id(edge_index, blacklist)) continue;
      uint32_t hits = hit_bits[edge_index];
      if(hits==0) {
        continue;
      }
      if(hits>CO_HITCOUNT_THRESHOLD * MIN_NUMHIT) {
        continue;
      }
      if(branch_flag[edge_index] == 1) {
        continue;
      }

      int insert_index = 0;
      for (int i = 0; i < MAX_DATA_SUFFIX; i++) {
          if (branch_ids_hits[i] == 0 || branch_ids_hits[i] > hits) {
              insert_index = i;
              break;
          }
          else {
      ...
      if (insert_index < MAX_DATA_SUFFIX) {
        for (int i = MAX_DATA_SUFFIX - 1; i > insert_index; i--) {
            branch_ids_hits[i] = branch_ids_hits[i - 1];
            branch_ids[i] = branch_ids[i - 1];
        }
        branch_ids_hits[insert_index] = hits;
        branch_ids[insert_index] = edge_index;
      }
  ...
  ck_free(branch_ids_hits);

  return branch_ids;
}

static u32 * is_target_branch_ids(u8* branch_trace, int target_index){
  if(branch_trace==NULL){
    return NULL;
  }
  u32 * branch_ids = ck_alloc(sizeof(u32) * MAX_RARE_BRANCHES + 1);
  int min_hit_index = 0;

  for (int i = 0; i < targets_branch_num_target[target_index];i++){
    if((branch_trace[i] ) == 1){
      u32 edge_index = order_edge_target[target_index][i];
      DEBUG1("edge_index: %d\n", edge_index);
      if (contains_id(edge_index, blacklist)) {
        continue;
        }
      int hits = hit_bits[edge_index];
      if(hits==0) {
        continue;
      }
      if(hits>CO_HITCOUNT_THRESHOLD * MIN_NUMHIT) {
      ...
      branch_ids[min_hit_index] = edge_index + 1;
      min_hit_index++;
    }
...
DEBUG1("this seed target branch index num:%d\n", min_hit_index);
...
static u8 fuzz_one(char** argv) {
  ...
  if (queue_cur->depth > 1) return 1;
    ...
    int rearest_target=  get_rarest_target();
    int test_target = rearest_target;

    u32 *target_branch_hit = is_target_branch_ids(queue_cur->target_branch_trace_target[rearest_target],rearest_target);
    if(target_branch_hit == NULL){
      for (int i = 0; i < targets_num;i++){
        if (target_count[i] < NOTRARE_LIMIT){
          target_branch_hit = is_target_branch_ids(queue_cur->target_branch_trace_target[i], i);
        }
        if(target_branch_hit == NULL){
          continue;
        }else{
    ...
    if(explore_status && queue_cur->targets){
      int i = UR(targets_num);
      if( queue_cur->targets[i]>0){
        target_branch_hit = is_target_branch_ids_data(queue_cur->target_branch_trace_target_data[i],i);
        test_target = i;
      }
    }
    ...
    if(target_branch_hit == NULL && run_in_Q2 && (queue_cur->ttarget_excutes == 0)){
      return 1;
    }
    if(target_branch_hit ==NULL && !explore_status){
      return 1;
    }
...
/* select inputs which hit rare branches */
    ...
    if (target_branch_hit == NULL){
      ...
      int ii;
      for (ii = 0; target_branch_hit[ii] != 0; ii++){
        rb_fuzzing = target_branch_hit[ii];
        DEBUG1("rb_fuzzing:%d\n", rb_fuzzing);
        ...
        if (rb_fuzzing){
          ...
          if (queue_cur->fuzzed_branches[byte_offset] & (1 << (bit_offset))){
            ...
            DEBUG1("queue_cur:%s, byte_offset: %d, 1<<bit_offset: %d\n", queue_cur->fname, byte_offset, 1 << (bit_offset));
            if(byte_offset > (MAP_SIZE>>3)){
              DEBUG1("out byte_offset!!!\n");
              exit(1);
            }
      ...
      if (!rb_fuzzing || (target_branch_hit[ii] == 0)){
        rb_fuzzing = target_branch_hit[0];
        if (!rb_fuzzing) {
          return 1;
        }
  ...
  orig_perf = perf_score = calculate_score(queue_cur,test_target);
  ...
  if ((!rb_fuzzing && skip_simple_bitflip && !queue_cur->is_target)
      || skip_deterministic_bootstrap
      // || queue_cur->was_fuzzed 
      || ((explore_status && !queue_cur->passed_det) 
        && perf_score < (
...
// DEBUG1("%scalib stage: %i new branches in %i total execs\n", shadow_prefix, queued_with_cov-orig_queued_with_cov, total_execs-orig_total_execs);
  ...
  for (stage_cur = 0; stage_cur < (len << 3) - 3; stage_cur++) {
    ...
    if (rb_fuzzing){//&& use_mask()){
      ...
      // if we're spilling into next byte, check that that byte can
      // be modified
      if ((stage_cur_byte != ((stage_cur + 3)>> 3))&& (!(branch_mask[stage_cur_byte + 3] & 1))){
        stage_max--;
        continue;
      }
    }
...
havoc_stage:
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    if((temp_len == len) && (!memcmp(in_buf,out_buf,temp_len))){
      not_havoc_exec++;
    }else{
...
abandon_entry:
  ...
  /* @RB@ reset stats for debugging*/
// DEBUG1("%sIn havoc stage, %i of %i tries hit branch %i,perf_score:%u,not_havoc_exec:%u\n", shadow_prefix, successful_branch_tries, total_branch_tries, rb_fuzzing - 1,perf_score,not_havoc_exec);
  successful_branch_tries = 0;
  total_branch_tries = 0;
...
void readSuffix(){
  target_suffix = ck_alloc(targets_num * sizeof(u32));
  // suffix_map = ck_alloc(targets_num * sizeof(u32));

  FILE *suffix_file = fopen("/home/suffix.txt", "r");
  char *buf[1024];
  ...
  while(fgets(buf,sizeof(buf),suffix_file)!=NULL){
    ...
    if (temp_target_id != temp_temp)
    {
      temp_temp = temp_target_id;
      target_id++;
    }
...
void readDistanceAndTargets() {
  ...
  memset((u8*)distance_val, 255, MAP_SIZE << 2);
  memset((u8*)branch_distance, 255, MAP_SIZE << 2);

  FILE* distance_file = fopen("/home/distance.txt","r");
  FILE* targets_file = fopen("/home/targets.txt","r");
  FILE* targetsidorig_file = fopen("/home/targets_id_orig.txt","r");
  
  FILE* targets_order_file = fopen("/home/branch-distance-order.txt","r");
  FILE* branch_distance_file = fopen("/home/branch-distance.txt","r");
  FILE* curloc_file = fopen("/home/branch-curloc.txt","r");

  FILE *targetid_file = fopen("/home/targets_id.txt", "r");
 
  char buf[1024]; 
  if (targetid_file==NULL)
    FATAL("targets_id.txt not exist");

  int i = 0;
  int target_ids_num = 0;
  while(fgets(buf,sizeof(buf),targetid_file)!=NULL){
    target_ids[i] = atoi(buf);
    i++;
  }
  target_ids_num = i;
  OKF("target_ids_num %d", target_ids_num);

  for (int i = 0; i < target_ids_num; i++)
  {
    char prefix[] = "/home/";
    char suffix[] = "-distance-order.txt";
    char filename[100]={'\0'};
    char num_str[20];
    strcat(filename, prefix);
    sprintf(num_str, "%d", target_ids[i]);
    strcat(filename, num_str);
    strcat(filename, suffix);
    OKF("this filename: %s", filename);

    FILE *targetid_file_target = fopen(filename, "r");
    if (targetid_file_target==NULL)
      FATAL("%s not exist",filename);
    
      
    while(fgets(buf, sizeof(buf), targetid_file_target) != NULL) {
          
      char* token;
      token = strtok(buf, " ");
      uint32_t edge_index = atoi(token);
      token = strtok(NULL, " ");
      uint32_t order = atoi(token);
      if(order < MAX_RARE_BRANCHES){
        order_edge_target[i][order] = edge_index;
      }else{
        break;
      }
      targets_branch_num_target[i]++;
    }
    OKF( "targets_branch_num_target[%d]:  %d\n",target_ids[i],targets_branch_num_target[i]);
    fclose(targetid_file_target);
  }
  ...
  {
    char prefix[] = "/home/";
    char suffix[] = "_edge_suffix_data-order.txt";
    char filename[100]={'\0'};
    char num_str[20];
    strcat(filename, prefix);
    sprintf(num_str, "%d", target_ids[i]);
    ...
    FILE *targetid_file_target_data = fopen(filename, "r");
    if (targetid_file_target_data==NULL)
      FATAL("%s not exist",filename);
    
      
    while(fgets(buf, sizeof(buf), targetid_file_target_data) != NULL) {
      ...
      uint32_t order = atoi(token);
      if(order < MAX_DATA_SUFFIX){
        order_edge_target_data[i][order] = edge_index;
      }else{
        break;
      }
      targets_branch_num_target_data[i]++;
    }
    OKF( "targets_branch_num_target_data[%d]:  %d\n",target_ids[i],targets_branch_num_target_data[i]);
    fclose(targetid_file_target_data);
  }
  ...
  if (targetsidorig_file==NULL)
    FATAL("targets_id_orig.txt not exist");

  while(fgets(buf, sizeof(buf), targetsidorig_file) != NULL) {
    targets_num_orig++;
  }
  if (targets_num_orig > TARGETS_NUM_ORIG ){
    targets_num_orig = TARGETS_NUM_ORIG;
  }

  if (targets_order_file==NULL)
    FATAL("branch-distance-order.txt not exist");

  while(fgets(buf, sizeof(buf), targets_order_file) != NULL) {
    ...
    uint32_t order = atoi(token);
    if(order < MAX_RARE_BRANCHES){
      order_edge[order] = edge_index;
    }else{
      break;
    }
    targets_branch_num++;
  }
  SAYF(cCYA "targets_branch_num " cBRI VERSION cRST " %d\n",targets_branch_num);
  ...
  while(fgets(buf, sizeof(buf), curloc_file) != NULL) {
    ...
    token = strtok(NULL, " ");
    uint32_t distance_temp = atoi(token);

    if((edge_curloc[edge_index] != 0) && (edge_distance[edge_index] > distance_temp)){
      edge_distance[edge_index] = distance_temp;
      edge_curloc[edge_index] = cur_loc;
    }else if(edge_curloc[edge_index] == 0){
      edge_distance[edge_index] = distance_temp;
      edge_curloc[edge_index] = cur_loc;
    }

    struct CE* temp = ck_alloc(sizeof(struct CE));
    temp->edgeidx=edge_index;
    temp->next = NULL;
    struct CE *t = curloc_edge[cur_loc];
    curloc_edge[cur_loc] = temp;
    temp->next = t;
  }

  if (branch_distance_file==NULL)
    FATAL("targets.txt not exist");
  while(fgets(buf, sizeof(buf), branch_distance_file) != NULL) {
    char* token;
    token = strtok(buf, " ");
    token = strtok(NULL, " ");
    int branch_idx = atoi(token);
    token = strtok(NULL, " ");
    token = strtok(NULL, " ");
    int dis = atoi(token);
    branch_distance[branch_idx] = dis < branch_distance[branch_idx] ? dis : branch_distance[branch_idx];
    token = strtok(NULL," ");
  }   


  fclose(branch_distance_file);
  fclose(distance_file);
  fclose(targets_file);
  fclose(targets_order_file);  
  fclose(curloc_file);
}
...
int main(int argc, char** argv) {
  ...
  SAYF(cCYA "waflgo-fuzz " cBRI VERSION cRST " by xy\n");
  ...
  while ((opt = getopt(argc, argv, "+bq:rsi:o:f:m:t:T:dnCB:S:M:x:Qz:c:")) > 0)
  ...
  ACTF("Validating ttarget binary...");
  ...
  while (1) {
    ...
    DEBUG1("Entering Q2\n");
      run_in_Q2 = 1;
      if(!queue_temp){
        queue_temp = queue_cur;
        temp_current_entry = current_entry;
      }

      queue_cur = queue2->queue_this;
      current_entry = queue_cur->number;
      DEBUG1(" queue_cur : %s\n", queue_cur->fname);
      struct Q2 *temp = queue2;
      queue2 = queue2->queue_next;
      ck_free(temp);

      unset_target_branch_flag(queue_cur->queue_target_branch_hit);
    }
    else {        
    DEBUG1("Entering Q11111 cycle\n");
      run_in_Q2 = 0;
      if(!queue_temp){
        queue_cur = queue_cur->next;
      }else{
        queue_cur = queue_temp->next;
        queue_temp = NULL;
        current_entry = temp_current_entry;
        temp_current_entry = 0;
      }
      current_entry++;
    }
