...
u8  __afl_area_initial[MAP_SIZE + 16 + 16 + 16 + 16 + TARGETS_NUM * 16 + TARGETS_NUM + MAX_RARE_BRANCHES * (TARGETS_NUM+1) + MAX_DATA_SUFFIX * (TARGETS_NUM+1)];
u8  __afl_area_initial_c[MAP_SIZE + 16 + 16 + 16 + 16];
u8  __afl_area_initial_suf[MAP_SIZE + 16 + 16 + 16 + 16];
u8  __afl_area_initial_bb[MAP_SIZE + 16 + 16 + 16 + 16];

u8* __afl_area_ptr = __afl_area_initial;
u8* __critical_bb_ptr = __afl_area_initial_c;
...
static void __afl_map_shm(void) {
  ...
  u8 *id_str_c = getenv(SHM_ENV_VAR_C);
  ...
  if (id_str && id_str_suf && id_str_suf_bb && id_str_c ) {

    u32 shm_id = atoi(id_str);
    u32 shm_id_c = atoi(id_str_c);
    u32 shm_id_suf = atoi(id_str_suf);
    u32 shm_id_suf_bb = atoi(id_str_suf_bb);

    __afl_area_ptr = shmat(shm_id, NULL, 0);
    __critical_bb_ptr = shmat(shm_id_c, NULL, 0);
...
int __afl_persistent_loop(unsigned int max_cnt) {
  ...
  if (first_pass) {
    ...
    if (is_persistent) {
      ...
      memset(__afl_area_ptr, 0, MAP_SIZE + 16 + 16 + 16 + 16 + TARGETS_NUM * 16 + TARGETS_NUM + MAX_RARE_BRANCHES * (TARGETS_NUM+1) + MAX_DATA_SUFFIX * (TARGETS_NUM+1));
      memset(__critical_bb_ptr, 0, MAP_SIZE + 16 + 16 + 16 + 16);
      memset(__afl_area_ptr_suf, 0, MAP_SIZE + 16 + 16 + 16 + 16);
      memset(__afl_area_ptr_suf_bb, 0, MAP_SIZE + 16 + 16 + 16 + 16);
  ...
  if (is_persistent) {
    ...
    if (--cycle_cnt) {
      ...
      __critical_bb_ptr = __afl_area_initial_c;
